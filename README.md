...
4. 为hadoop用户赋予sudo权限和NOPASSWD权限
5. 为每个节点配置hostname和ip并保持和配置文件的hosts一致
6. 发送文件到master节点并`chmod 755`（slave节点会自动执行`chmod`）
7. 在master上执行`run-on-master.sh`
8. 安装完成后为每个机器执行`source ~/.bashrc`
9. 目前可能需要完成后在master节点执行`ssh hadoop@$hostname`以保证正确输入`yes`（先运行start-*.sh可能无法输入`yes`）
